   ...
   let id = (this.props.navigation.state.params.product.product_id != null) ? this.props.navigation.state.params.product.product_id : this.props.navigation.state.params.product.id
	...
	apis.getProduct(id).then(res => {
      ...
      if(this.state.user != null){
        link = link + '?login=1&key='+this.state.key;
      }else{
        link = link + '?login=0&key=0';
      }
    ...
    return (
      <View style={styles.wrapper}>
         ...
         <Header navigation={this.props.navigation} />
          <ScrollView contentInsetAdjustmentBehavior="automatic" style={styles.scrollview_section} ref='_scrollView'>
		...
		onMessage={this.onWebViewMessage}
         ...
         source={{ uri: this.state.link }} />
							...
							<TouchableOpacity style={[styles.plus, {position: 'absolute', top: width + 259, right: 10}]} onPress={this._document}>
	                            <Image
